...
Skeletor.Utilities.autoSubscribe = function(instance, schemaName, type) {
    ...
    if (type === 'detail') {
        ...
        instance.autorun(() => {
            ...
            if (subManager) {
                Skeletor.subsManagers[subManager].subscribe('findDocuments', collection, query, options, schemaName);
                readyHandles.push(Skeletor.subsManagers[subManager]);
            }
            else {
                Meteor.subscribe('findDocuments', collection, query, options, schemaName);
                unCachedSubs = true;
            }

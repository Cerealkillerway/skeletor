Meteor.methods({
    ...
    insertDocument: function(data, schemaName) {
        ...
        // check permissions
        if (!SkeleUtils.GlobalUtilities.checkPermissions(Skeletor.Utilities.getPermissions(collection, 'create'))) {
    ...
    updateDocument: function(documentId, data, schemaName) {
        ...
        // check permissions
        if (!SkeleUtils.GlobalUtilities.checkPermissions(Skeletor.Utilities.getPermissions(collection, 'update', {documentId: documentId, data: data}))) {
            throw new Meteor.Error('unauthorized');
        }
        /*else {
            if (Meteor.isServer) {
    ...
    deleteDocument: function(documentId, schemaName) {
        ...
        // check permissions
        if (!SkeleUtils.GlobalUtilities.checkPermissions(Skeletor.Utilities.getPermissions(collection, 'delete'))) {
            throw new Meteor.Error('unauthorized');
        }
        /*else {
            if (Meteor.isServer) {
    ...
    insertUser: function(data, schemaName) {
        ...
        // check permissions
        if (!SkeleUtils.GlobalUtilities.checkPermissions(Skeletor.Utilities.getPermissions('users', 'create'))) {
            throw new Meteor.Error('unauthorized');
        }
        /*else {
            if (Meteor.isServer) {
    ...
    // update
    updateUser: function(documentId, data, schemaName) {
        'use strict';
        var schema = Skeletor.Schemas[schemaName];

        // check permissions
        if (!SkeleUtils.GlobalUtilities.checkPermissions(Skeletor.Utilities.getPermissions('users', 'update', {documentId: documentId, data: data}))) {
            throw new Meteor.Error('unauthorized');
        }
        /*else {
            if (Meteor.isServer) {

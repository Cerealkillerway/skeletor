Meteor.methods({
    ...
    updateUser: function(documentId, data, schemaName, changed) {
        ...
        if (data.password !== undefined) {
            var password = {digest: data.password, algorithm: 'sha-256'};
            
            delete data.password;
            //if (Meteor.isClient) Router.current().state.set('forcedLogout', true);
        }
        ...
        Meteor.users.update({_id: documentId},{$set: data}, function(error, number) {
            if (error) return error;
            if ((password !== undefined) && (Meteor.isServer)) Accounts.setPassword(documentId, password);
            return number;
        }); 
    },
    // delete

...
function checkAuth(current) {
    if (!Meteor.userId() || !checkPermissions("panelAccess", Meteor.userId())) {
        return FlowRouter.go(Skeletor.configuration.login.defaultLoginPath);
    }    
}
...
FlowRouter.route("/panel", {
    ...
    action: function(params, queryParams) {
        FlowRouter.go("/panel/dashboard", {}, {lang: Session.get("currentLang")});
    }
});
...
PanelRoutes.route('/users/create', {
    subscriptions: function() {
        //load usernameList to be able to check for already taken usernames
        return [
            subman.subscribe("findUsers", {}, {username: 1, roles: 1}, "adminOperations"),
            subman.subscribe("findDocuments", "Roles", {}, {})
        ];
    },
    data: function() {
        var context = {};
